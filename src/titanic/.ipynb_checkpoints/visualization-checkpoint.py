...
def set_plot_style():
    ...
    sns.set_palette(['#4C72B0', '#DD8452', '#193760', '#0b2447'])
...
def plot_count_pairs(df, column: str, hue_column: str = None, title: str = None, figsize: tuple = (8, 4)):
    ...
    if hue_column:
        sns.countplot(x=column, data=df, hue=hue_column, palette=["#4C72B0", "#DD8452"])
    else:
        sns.countplot(x=column, data=df, color="#4C72B0")

    plt.grid(color="gray", linestyle="-.", linewidth=0.5, axis="y", which="major")
